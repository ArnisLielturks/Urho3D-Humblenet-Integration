	...
	switch (msgType) {
		case HumblePeer::MessageType::P2POffer:
		{
			...
			if (emulated || !humbleNetState.webRTCSupported) {
				...
				LOG(">>> EMULATED %d, humbleNetState.webRTCSupported %d", emulated, humbleNetState.webRTCSupported);
			...
			if( ! ret )
			{
			    LOG("\n>>>> internal_set_offer returned false!!!");
		...
		case HumblePeer::MessageType::HelloClient:
		{
			...
			if (hello->iceServers()) {
				...
				for (const auto& it : *iceList) {
					if (it->type() == HumblePeer::ICEServerType::STUNServer) {
                        ...
                        LOG("TURN server received %s, %s, %s\n", it->server()->c_str(), username->c_str(), pass->c_str());
						if (pass && username) {
            ...
            std::string turnServerAddress;
            std::string turnServerUser;
            std::string turnServerPassword;
			for (auto& it : humbleNetState.iceServers) {
				if (it.type == HumblePeer::ICEServerType::STUNServer) {
					stunServers.emplace_back(it.server.c_str());
				} else if (it.type == HumblePeer::ICEServerType::TURNServer) {
//                    turnServers.emplace_back(it.server.c_str());
                    turnServerAddress = it.server;
                    turnServerUser = it.username;
                    turnServerPassword = it.password;
				}
			}
			internal_set_stun_servers(humbleNetState.context, stunServers.data(), stunServers.size());
			if (!turnServerAddress.empty()) {
			    LOG("Setting received turn server %s;%s:%s", turnServerAddress.c_str(), turnServerUser.c_str(), turnServerPassword.c_str());
                internal_set_turn_server(humbleNetState.context, turnServerAddress.c_str(), turnServerUser.c_str(), turnServerPassword.c_str());
            }
		}
			break;
